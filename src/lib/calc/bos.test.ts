...
): Array<{ date: string; low: number; high: number; close?: number }> {
...
// L1 (安値40, index6) → H1 (高値75, index13) → L2 (安値50, index20、L1より高い) →
// 回復 (index30まで、H1=75を上抜ける)
const TYPICAL_WAYPOINTS = [
  { index: 0, low: 65 },
  { index: 6, low: 40 }, // L1
  { index: 13, low: 70 }, // H1 (high = 75)
  { index: 20, low: 50 }, // L2 (L1 より高い = 安値切り上げ)
  { index: 30, low: 78 }, // 回復
];

describe('detectBos (#272)', () => {
  it('上昇構造 (L1→H1→L2、安値切り上げ) の後、終値がH1を上抜けたら検出する', () => {
    const result = detectBos(bars(wave(TYPICAL_WAYPOINTS)), 3);
    expect(result).not.toBeNull();
    expect(result!.firstLowDate).toBe(d(6));
    expect(result!.firstLowPrice).toBe(40);
    expect(result!.priorHighDate).toBe(d(13));
    expect(result!.priorHighPrice).toBe(75);
    expect(result!.secondLowDate).toBe(d(20));
    expect(result!.secondLowPrice).toBe(50);
    expect(result!.swingLength).toBe(3);
    expect(result!.breakoutPrice).toBeGreaterThan(75);
  });

  it('2つ目の安値が1つ目以下 (安値切り上げでない) なら検出しない', () => {
    const notHigher = TYPICAL_WAYPOINTS.map((w) => (w.index === 20 ? { ...w, low: 30 } : w));
    expect(detectBos(bars(wave(notHigher)), 3)).toBeNull();
  });

  it('前回の高値をまだ上抜けていなければ検出しない', () => {
    const noBreakout = TYPICAL_WAYPOINTS.map((w) => (w.index === 30 ? { ...w, low: 60 } : w));
    expect(detectBos(bars(wave(noBreakout)), 3)).toBeNull();
  });

  it('CHoCH向けの下落構造 (安値切り下げ) はBOSとして検出しない', () => {
    // choch.test.ts の TYPICAL_WAYPOINTS と同じ (L2 が L1 より低い)。
    const chochShape = [
      { index: 0, low: 65 },
      { index: 6, low: 50 },
      { index: 13, low: 70 },
    ...
    expect(detectBos(bars(wave(chochShape)), 3)).toBeNull();
  });
});
