...
const CHARTS: QuestionChartDef<MarketQuestionId>[] = [
  ...
  },
  {
    ...
    title: 'カップウィズハンドル',
    subtitle:
      '上昇 → 12〜33% の押し目 (7〜65 週、左側が右側より長い) → 右肩の回復 → 1〜4 週の保ち合いを形成している銘柄',
    notes: [
      <span key="def">
        <strong>William O&apos;Neil の定義。</strong>
        「カップ」は深さ 12〜33% (IBD 公式資料の定義、1/2 まで許容、Dow 理論に基づき最大
        2/3)、期間は 7〜65 週。左側 (下落局面) は右側 (回復局面) より長いのが正しい形
        (Martinelli &amp; Hyman, 1998, Stocks &amp; Commodities 誌)。「ハンドル」は右肩
        (カップ完成) から 1〜4 週間、カップの上半分に留まり、右肩からの下落が直近上昇幅の
        1/3 以内の保ち合い。
      </span>,
      <span key="shape">
        <strong>「U 字型」の形状条件そのものは判定に含めていない。</strong>
        一次情報は定性的にしか説明しておらず、定量化する明確な基準が見当たらないため、
        恣意的な閾値を追加で作らず、深さ・期間・左右の時間比率・ハンドルの条件で判定している。
        検出数はこれらの条件を重ねるほど絞り込まれる (実データで 36 銘柄中 34 → 数銘柄まで
        減少)。
      </span>,
      <span key="rare">
        <strong>極めて稀なパターン。</strong>
