...
const CHARTS: QuestionChartDef<MarketQuestionId>[] = [
  ...
  },
  {
    ...
    primaryIndicator: 'etf-spy',
    breakout: true,
    title: 'ブレイクアウト (N日高値抜け)',
    subtitle: '直近 20 営業日の高値を終値が上抜けた銘柄',
    notes: [
      <span key="def">
        <strong>Donchian チャネル/タートルズの短期システムに準拠した定義。</strong>
        直近 20 営業日の高値 (当日を含まない) を、当日の終値が上回っていることを検出する。
      </span>,
      <span key="why">
        <strong>反転パターンの形状検出 (カップウィズハンドル・ダブルボトム・逆三尊・
          High Tight Flag) から置き換えた。</strong>
        形状を数値条件で判定するアプローチは、条件を厳格化するたびに検出数が乱高下し
        (カップウィズハンドルは実データで 36 銘柄中 34 → 2 まで変動)、実際のチャートを
        見ると判定した形に見えないことがあった。確立された定義を持つ単純な
        ブレイクアウトに一本化している。
      </span>,
      <span key="rare">
