 ...
 *
 * CHoCH (`BreakoutList` #268) と対になる。**検出された銘柄のみ** `PatternChart` (#260)
 * で上昇構造の主要点 (1つ目の安値・前回の高値・2つ目の安値・ブレイクアウト) を視覚化する。
 * 0 件はテキストのみ (0 件は正常な状態 — ビューが生成されていること自体が
 * 「検出を試みた」証拠になる)。
 */
export function BosList({
  title,
  subtitle,
  assets,
  notes,
  ...
  const detected = assets.filter(
    (asset): asset is BosAsset & { detection: NonNullable<BosAsset['detection']> } =>
