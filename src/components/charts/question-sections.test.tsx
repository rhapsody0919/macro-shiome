...
describe('街角景気 (#160)', () => {
  it('景気動向指数と同じ図に載せない', () => {
    ...
    const section = document.getElementById('q-jp-cycle');
    const titles = Array.from(section?.querySelectorAll('h3') ?? []).map((h) => h.textContent);
    // #226 で企業の景況感 (BSI) が加わって 5 枚、#336 で実質GDP成長率が加わって 6 枚、
    // #228 で日銀短観が加わって 7 枚。
    // 街角景気とは情報源が別 (こちらは経営側、あちらは小売・飲食などの現場) なので重ねない。
...
describe('日本の景気動向指数 (#154)', () => {
  it('先行・一致・遅行を 1 枚に重ねる', () => {
    ...
    // #228 で日銀短観が BSI の直後に加わった (同じ「企業」の体感を補う調査のため)。
    // #336 で実質GDP成長率が最後に加わった。月次の動きの答え合わせなので最下流に置く。
    expect(titles).toEqual([
      '景気動向指数',
      '企業の景況感 (BSI)',
      '日銀短観 業況判断DI (大企業)',
