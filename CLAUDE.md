...
| UI/UX | #74〜#77 | 期間フィルター修正 / モバイル / サマリー / 情報設計 |
...
| `/macro` | 週次 | 市場データ 10 チャート。金利・信用スプレッド・新規求人・失業保険申請・VIX・商品・為替 |
| `/economy` | **月次** | 経済統計 12 チャート。住宅・新規受注・労働時間・生産・雇用・所得・消費・物価・財政 |
| `/terms` | — | 利用規約・出所 |

**ページは更新頻度で分ける** (#64)。週次と月次を混ぜると「先週から動いていない」のか
「月次だから動かない」のか読み手が判別できない。
**セクションは景気サイクルで分ける** (#62)。分類は The Conference Board の構成要素が根拠。
...
### UI/UX (2026-08-17、#74〜#77 で改善)

実画面 (375px / 768px / 1280px) で調査し、4 件を修正した。
...
**画面の原則**:
- ページは**更新頻度**で分ける (週次 / 月次)
- セクションは**景気サイクル**で分ける (先行 / 一致 / 遅行 / その他)
- サマリーは**期間フィルターに依存しない**。常に直近を見せる
- 月次のカードは**指標ごとに対象月を出す**。発表ラグが違うため

### 未解決の設計課題 (#89)

指標が増えた結果、**「その他の指標」が最大セクションになった** (`/macro` は 82%)。
#62 の「構成要素を根拠に分類し、当てはまらないものは分類しない」方針は正しいが、
投資判断で見る指標の大半は景気指数の構成要素ではないため増やすほど溜まる。
案 A (テーマ別サブグループ) / B (軸をテーマに変更) / C (ページ分割) を #89 に整理済み。

### Next Action

- **#89 (「その他」の肥大化) — ユーザー判断待ち**
